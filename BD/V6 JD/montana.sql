...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-08-2018 a las 02:15:12
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ListaDetXturno` (IN `turno` INT)  NO SQL
SELECT "Pedido" concepto,idPedido codigo,B.Documento,B.NombreCompleto,A.FechaRegistro,A.FechaEntrega FROM pedido A, persona B where A.idPersonacliente=B.idPersona and   idTurno=turno
union ALL
SELECT "Despacho" concepto ,A.idDespacho codigo,C.documento,C.NombreCompleto,B.FechaRegistro,A.FechaEntrega FROM despacho A, pedido B,persona C  WHERE A.idPedido=B.idpedido and B.idPersonacliente=C.idpersona and A.idTurno=turno$$
...
INSERT INTO `despacho` (`idDespacho`, `idTurno`, `Usuario`, `idPersona`, `idPedido`, `factura`, `FechaEntrega`, `idCiudad`, `NumeroGuia`, `idTipoDespacho`) VALUES
(3, 33, 'admon', 1, '14', '', '2018-08-06', '0', '', 1);
...
--
-- Volcado de datos para la tabla `despachoproducto`
--

INSERT INTO `despachoproducto` (`idDespachoProducto`, `idDespacho`, `idProducto`, `idTalla`, `cantidad`, `CantidadPedido`) VALUES
(5, 3, 3, 8, 15, 0),
(6, 3, 68, 3, 8, 0),
(7, 3, 4, 2, 150, 0);
...
(2, 'Despacho Parcial', 'A'),
(3, 'En Pie', 'A'),
(4, 'Completado', 'A'),
(5, 'Cancelado', 'A'),
(6, 'Despacho Adicional', 'A');
...
('Cajas', 1, 99999, 78, 1, 'A'),
('producto', 1, 1000, 94, 1, 'A');
...
CREATE TABLE `pedido` (
  ...
  `UsuarioTurno` varchar(15) NOT NULL,
  `idPersonaTurno` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `pedido` (`idPedido`, `idCliente`, `idPersonacliente`, `FechaRegistro`, `FechaEntrega`, `idTurno`, `Usuario`, `Observacion`, `total_pedido`, `UsuarioTurno`, `idPersonaTurno`) VALUES
(2, 1, 5, '2018-08-01 17:28:36', '2018-07-25 05:00:00', 1, 'admon', 'xxxx', 0, '', 0),
(3, 1, 5, '2018-08-01 17:28:39', '2018-07-31 22:10:14', 33, 'admon', '', 0, '', 0),
(4, 1, 5, '2018-08-01 17:28:41', '2018-07-31 22:10:14', 33, 'admon', '', 0, '', 0),
(5, 1, 5, '2018-08-01 20:20:28', '2018-08-01 20:25:24', 33, 'admon', '', NULL, '', 0),
(6, 1, 5, '2018-08-01 20:20:41', '2018-08-01 20:25:24', 33, 'admon', '', NULL, '', 0),
(9, 1, 5, '2018-08-01 20:37:05', '2018-08-01 20:41:53', 33, 'admon', '', NULL, '', 0),
(10, 1, 5, '2018-08-01 20:37:11', '2018-08-01 20:41:53', 33, 'admon', '', NULL, '', 0),
(11, 1, 5, '2018-08-01 21:20:56', '2018-08-01 21:29:00', 33, 'admon', '', NULL, '', 0),
(12, 1, 5, '2018-08-01 21:21:07', '2018-08-01 21:29:00', 33, 'admon', '', NULL, '', 0),
(13, 1, 5, '2018-08-01 21:21:44', '2018-08-01 21:29:00', 33, 'admon', '', NULL, '', 0),
(14, 1, 5, '2018-08-07 17:18:04', '2018-08-01 21:29:00', 34, 'admon', '', NULL, '', 0),
(15, 1, 5, '2018-08-07 17:18:00', '2018-08-01 22:06:22', 34, 'admon', '', NULL, '', 0);
...
--
-- Volcado de datos para la tabla `pedidovariables`
--

INSERT INTO `pedidovariables` (`idPedidoVariable`, `idPedido`, `idCliente`, `idPersona`, `id_variables`, `Estado`, `fecha`, `idDespacho`) VALUES
(2, 14, 1, 1, 1, 'A', '2018-08-06 12:51:31', 3);

--
-- Disparadores `pedidovariables`
--
DELIMITER $$
CREATE TRIGGER `upEstauspedidodetalle` AFTER INSERT ON `pedidovariables` FOR EACH ROW update pedido_detalle pdetalle 
inner join pedidovariables p  on pdetalle.idPedido = p.idPedido 
INNER join despacho d on d.idDespacho = p.idDespacho 
inner join despachoproducto dp on d.idDespacho = p.idDespacho 
set pdetalle.despachado = 
case when p.id_variables IN (1,6) AND dp.cantidad < pdetalle.cantidad then 'DP' 
     when p.id_variables IN (1,6) AND dp.cantidad > pdetalle.cantidad then 'DA'
	 when p.id_variables IN (1,6) AND dp.cantidad = pdetalle.cantidad then 'DC'	 
	 when p.id_variables = 2 AND dp.cantidad < pdetalle.cantidad then 'DP'
	 when p.id_variables = 2 AND dp.cantidad > pdetalle.cantidad then 'DA'
	 when p.id_variables = 2 AND dp.cantidad = pdetalle.cantidad then 'DC'
	 when p.id_variables = 3 AND dp.cantidad < pdetalle.cantidad then 'DP'
	 when p.id_variables = 3 AND dp.cantidad > pdetalle.cantidad then 'DA'
	 when p.id_variables = 3 AND dp.cantidad = pdetalle.cantidad then 'DC'
	 when p.id_variables = 4 AND dp.cantidad < pdetalle.cantidad then 'COM'
	 when p.id_variables = 4 AND dp.cantidad > pdetalle.cantidad then 'COM'
	 when p.id_variables = 4 AND dp.cantidad = pdetalle.cantidad then 'COM'
	 when p.id_variables = 5 AND dp.cantidad < pdetalle.cantidad then 'CAN'
	 when p.id_variables = 5 AND dp.cantidad > pdetalle.cantidad then 'CAN'
	 when p.id_variables = 5 AND dp.cantidad = pdetalle.cantidad then 'CAN'
end 
where p.idDespacho = NEW.idDespacho and p.idPedido = NEW.idPedido
and  pdetalle.idPedido = p.idPedido 
and pdetalle.idTalla = dp.idTalla and pdetalle.idProducto = dp.idProducto
and NEW.idDespacho > 0
$$
DELIMITER ;
...
(1, 'Admon', 'Admon', 'Admon Admon', '1111111', '2018-08-06', 'M', '11', '', 1, 'A'),
...
--
-- Volcado de datos para la tabla `tipodespacho`
--

INSERT INTO `tipodespacho` (`idTipoDespacho`, `Descripcion`, `estado`) VALUES
(1, 'Nomral', 'A'),
(2, 'Venta Directa', 'A');
...
(30, 'admon', 1, '2018-07-05 16:55:30', 'C'),
(31, 'admon', 1, '2018-07-05 16:59:45', 'C'),
(32, 'admon', 1, '2018-07-05 17:01:26', 'C'),
(33, 'admon', 1, '2018-07-15 23:07:01', 'C'),
(34, 'admon', 1, '2018-08-06 20:12:12', 'A');
...
--
ALTER TABLE `pedido`
  ADD PRIMARY KEY (`idPedido`,`idCliente`,`idPersonacliente`),
  ADD KEY `Cliente_Pedido_fk` (`idCliente`,`idPersonacliente`);
...
--
ALTER TABLE `cajaproducto`
  MODIFY `idCajaProducto` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;

--
-- AUTO_INCREMENT de la tabla `cajas`
--
ALTER TABLE `cajas`
  MODIFY `idCaja` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=76;
...
--
ALTER TABLE `despachoproducto`
  MODIFY `idDespachoProducto` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
...
--
ALTER TABLE `hormasprod`
  MODIFY `idHormaProd` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17;

--
-- AUTO_INCREMENT de la tabla `insumoproducto`
--
ALTER TABLE `insumoproducto`
  MODIFY `idInsumoProducto` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=34;
...
--
ALTER TABLE `mvariables`
  MODIFY `id_variables` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
ALTER TABLE `pedidovariables`
  MODIFY `idPedidoVariable` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
ALTER TABLE `pedido`
  ADD CONSTRAINT `Cliente_Pedido_fk` FOREIGN KEY (`idCliente`,`idPersonacliente`) REFERENCES `cliente` (`idCliente`, `idPersona`);
